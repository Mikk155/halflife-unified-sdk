...
bool CBaseHGruntAlly::CheckRangeAttack2(float flDot, float flDist)
{
	...
	else
	{
		...
		// estimate position
		if (HasConditions(bits_COND_SEE_ENEMY))
			vecTarget = vecTarget + ((vecTarget - pev->origin).Length() / GetSkillFloat("hgrunt_ally_gspeed"sv, 600)) * m_hEnemy->pev->velocity;
	}
	...
	{
		Vector vecToss = VecCheckThrow(this, GetGunPosition(), vecTarget, GetSkillFloat("hgrunt_ally_gspeed"sv, 600), 0.5);
...
void CBaseHGruntAlly::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_KICK:
	{
		...
		if (pHurt)
		{
			...
			pHurt->TakeDamage(this, this, GetSkillFloat("hgrunt_ally_kick"sv, 10), DMG_CLUB);
		}
	}
	break;
