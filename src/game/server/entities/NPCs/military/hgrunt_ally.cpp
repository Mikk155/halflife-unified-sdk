...
void CHGruntAlly::OnCreate()
{
	...
	pev->health = GetSkillFloat("hgrunt_ally_health"sv, 50);
	...
	if (m_hEnemy)
	{
		...
		else
		{
			...
			if (firstShotInBurst)
			{
				...
				FireBullets(GetSkillFloat("hgrunt_ally_pellets"sv, 5), vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, BULLET_PLAYER_BUCKSHOT, 0); // shoot +-7.5 degrees
