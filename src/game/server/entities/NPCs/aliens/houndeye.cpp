...
void CHoundeye::OnCreate()
{
	...
	pev->health = GetSkillFloat("houndeye_health"sv, 20);
		...
		if (pEntity->pev->takedamage != DAMAGE_NO)
		{
			...
			{ // houndeyes don't hurt other houndeyes with their attack
				...
				if (SquadCount() > 1)
				{
					...
					flAdjustedDamage = GetSkillFloat("houndeye_dmg_blast"sv, 15) + GetSkillFloat("houndeye_dmg_blast"sv, 15) * ( GetSkillFloat("houndeye_squad_bonus"sv, 1.1 ) * (SquadCount() - 1));
				}
				else
				{
					// solo
					flAdjustedDamage = GetSkillFloat("houndeye_dmg_blast"sv, 15);
				}
