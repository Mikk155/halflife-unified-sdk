...
// ============================================================ //
//					START OF TEMPORARY ENTITIES					//
// ============================================================ //

#define SF_TEMPENT_REPEATABLE  ( 1 << 0 )
#define SF_TEMPENT_ALL_PLAYERS ( 1 << 1 )	// All players (not activator) can see this FX
#define SF_TEMPENT_ACTIVATOR   ( 1 << 2 )	// Activator can see this FX

class CTempEntity : public CPointEntity
{
	DECLARE_CLASS( CTempEntity, CPointEntity );
	DECLARE_DATAMAP();
	...
	public:
		...
		Vector GetPosition( CBaseEntity* pActivator, CBaseEntity* pCaller );
		void ModelSet( const char* szModel );

		bool m_bIsThinking = false;	// Return whatever this entity is thinking
		bool m_bThinkEnt = false;	// Return whatever this entity has Think methods, do not update!
		int m_iModel;	// Model index for pev->model or default model
		string_t m_szModel;	// Model name for pev->model or default model
		Vector m_VecOffSet;	// OffSet vector to apply
};

BEGIN_DATAMAP( CTempEntity )
	DEFINE_FIELD( m_bThinkEnt, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bIsThinking, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_VecOffSet, FIELD_VECTOR ),
//	DEFINE_FIELD( m_szModel, FIELD_STRING ),
//	DEFINE_FIELD( m_szModel, FIELD_STRING ),
END_DATAMAP();
...
Vector CTempEntity :: GetPosition( CBaseEntity* pActivator, CBaseEntity* pCaller )
{
	Vector VecPos;

	if( !FStringNull( pev->netname ) )
	{
		CBaseEntity* pEntity = UTIL_FindEntityByTargetname( nullptr, STRING( pev->netname ), pActivator, pCaller );
		...
		{
			UTIL_StringToVector( VecPos, STRING( pev->netname ) );
		}
	}
	...
	if( !VecPos || VecPos == g_vecZero )
	{
		if( !FStringNull( pev->netname ) )
		{
			CBaseEntity::Logger->debug("Couldn't found netname {} for entity {} returning pev->origin", STRING( pev->netname ), STRING( pev->classname ) );
		}

		VecPos = pev->origin;
	}

	if( m_VecOffSet != g_vecZero )
	{
		if( m_VecOffSet.x > 0 ) VecPos.x += m_VecOffSet.x; else VecPos.x -= m_VecOffSet.x;
		if( m_VecOffSet.y > 0 ) VecPos.y += m_VecOffSet.y; else VecPos.y -= m_VecOffSet.y;
		if( m_VecOffSet.z > 0 ) VecPos.z += m_VecOffSet.z; else VecPos.z -= m_VecOffSet.z;
	}
...
void CTempEntTarExplosion::Use( CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value )
{
	Vector VecPos = GetPosition( pActivator, pCaller );
	...
	CTempEntity::Use( pActivator, pCaller, useType, value );
}
